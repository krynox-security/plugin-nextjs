...
- `<KrynoxCaptcha>` React component, `verifyKrynox()` for Route Handlers and Server
  Actions, and a `krynoxMiddleware()` factory for header-token API routes.
- Middleware omits `remoteip` by default; opt in with `resolveIp()` or an explicit
  `trustedProxyHeader` that your edge overwrites.

[0.1.0]: https://github.com/krynox-security/plugin-nextjs/releases/tag/v0.1.0
